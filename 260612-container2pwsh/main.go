...
func main() {
    cmd := exec.Command("powershell.exe", "-NoLogo", "-NoProfile", "-File", "win_script.ps1")
    cmd.Env = append(os.Environ(), "MY_CONTAINER_ENV=TestEnv")
    cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}

    // Перенаправлення виводу PowerShell у консоль Go
    ...
    memInfo.BasicLimitInformation.LimitFlags = 0x200 | 0x100
    ...
    fmt.Printf("✅ Контейнер завершив роботу успішно і зупинений о %s\n.", now)
}
