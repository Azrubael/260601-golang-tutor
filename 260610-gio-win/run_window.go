		...
		case app.FrameEvent:
			...
			layout.Flex{Axis: layout.Vertical, Spacing: layout.SpaceStart,
				...
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					return renderButton(gtx, theme, file_btn, "File", &file_open, &action_open, &help_open)
				}),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					return renderButton(gtx, theme, action_btn, "Action", &action_open, &file_open, &help_open)
				}),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					return renderButton(gtx, theme, help_btn, "Help", &help_open, &file_open, &action_open)
				}),
			)
...
func draw_button(g C, th *material.Theme, btn *widget.Clickable,
	actual_btn string) D {
	margins := layout.Inset{
		Top:    unit.Dp(5),
		Bottom: unit.Dp(0),
	...
	d := material.Button(th, btn, actual_btn).Layout(g)
	return margins.Layout(g,
		func(g C) D {
			return d
		},
	)
...
func renderButton(gtx C, theme *material.Theme, btn *widget.Clickable,
